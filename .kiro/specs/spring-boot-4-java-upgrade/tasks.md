...
- [ ] 1. Atualizar `pom.xml` para Java 25 e Spring Boot 4.0.0
  - [ ] 1.1 Atualizar parent BOM, `java.version` e remover versões explícitas dos starters
  ...
  - [ ] 1.2 Remover `hibernate-entitymanager` e substituir Springfox por springdoc-openapi
  ...
  - [ ] 1.3 Atualizar versões de jqwik e JaCoCo
...
- [ ] 2. Reescrever `SwaggerConfig` para springdoc-openapi
  - [ ] 2.1 Substituir implementação baseada em Springfox pelo bean `OpenAPI` do springdoc
...
- [ ] 3. Checkpoint — verificar compilação após mudanças no `pom.xml` e `SwaggerConfig`
  - Executar `mvnw.cmd clean package -DskipTests` e confirmar que a última linha contém `BUILD SUCCESS`
  - _Requisitos: 1.4, 4.6_

- [ ] 4. Migrar imports `javax.*` para `jakarta.*`
  - [ ] 4.1 Substituir `javax.persistence.*` por `jakarta.persistence.*` nas entidades JPA
  ...
  - [ ] 4.2 Substituir `javax.validation.*` por `jakarta.validation.*` nos DTOs
  ...
  - [ ] 4.3 Substituir `javax.servlet.*` por `jakarta.servlet.*` no handler de exceções
...
- [ ] 5. Checkpoint — verificar compilação após migração javax→jakarta
...
- [ ] 6. Ajustar `application.properties` para Spring Boot 4
  - [ ] 6.1 Remover propriedades `security.oauth2.*` incompatíveis com Spring Boot 4
...
- [ ] 7. Corrigir testes quebrados por mudanças de API do Spring Boot 4
  - [ ] 7.1 Substituir `@SpyBean` por `@MockitoSpyBean` em `ClienteResourceTest`
  ...
  - [ ] 7.2 Migrar `ClienteRepositoryPropertyTest` do padrão `@BeforeProperty` para `ApplicationContextAware`
  ...
  - [ ] 7.3 Verificar compatibilidade dos testes `@DataJpaTest` com jqwik 1.9.3
  ...
  - [ ] 7.4 Verificar compatibilidade dos testes `@SpringBootTest` com MockMvc e Jackson 3
...
- [ ] 8. Checkpoint — todos os testes existentes devem passar
...
- [ ] 9. Implementar property-based tests para as propriedades de corretude
  - [ ] 9.1 Criar classe `ClienteResourcePropertyTest` com configuração base
  ...
  - [ ]* 9.2 Implementar Propriedade 1: busca por ID existente retorna DTO completo
  ...
  - [ ]* 9.3 Implementar Propriedade 2: busca por ID inexistente retorna 404
  ...
  - [ ]* 9.4 Implementar Propriedade 3: criação com payload válido retorna 201 com Location
  ...
  - [ ]* 9.5 Implementar Propriedade 4: criação com email inválido retorna 422 com erros de validação
  ...
  - [ ]* 9.6 Implementar Propriedade 5: atualização com payload válido retorna 200
...
- [ ] 10. Checkpoint — todos os testes (incluindo property tests) devem passar
  - Executar `mvnw.cmd test` e confirmar `BUILD SUCCESS` com zero falhas
  - _Requisitos: 6.1_

- [ ] 11. Verificar cobertura JaCoCo
  - [ ] 11.1 Executar `mvnw.cmd verify` e confirmar cobertura LINE ≥ 70%
...
- [ ] 12. Atualizar steering files
  - [ ] 12.1 Atualizar `.kiro/steering/tech.md` com a nova stack
  ...
  - [ ] 12.2 Criar `.kiro/steering/migration-spring-boot-4.md` com registro da migração
