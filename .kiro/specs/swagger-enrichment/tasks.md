...
- [x] 6. Criar testes de smoke do bean `OpenAPI`
  - Criar classe de teste que carrega o contexto Spring e inspeciona o bean `OpenAPI`
  - _Requisitos: 2.1, 2.2, 2.3, 2.4_

  - [x] 6.1 Criar `SwaggerConfigTest` com testes de smoke do bean `OpenAPI`
...
- [x] 7. Criar testes de integração MockMvc para verificar roteamento preservado
  ...
  - [x] 7.1 Criar `ResourceRoutingIntegrationTest` para `ClienteResource` e `RoleResource`
  ...
  - [x] 7.2 Criar testes MockMvc para `VisitaResource` e `UserResource`
